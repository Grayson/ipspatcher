...
public struct Hunk {
    ...
    var isRLE: Bool { return isValidRLEHunk }
    var RLELength: UInt16 { return UInt16(payload[0]) << 16 + UInt16(payload[1]) }
    var RLEPayload: UInt8 { return payload[2] }

    var sizeInBytes: Int {
        return 3 /* offset */ + 2 /* length */ + payload.count
    }

    static func from(bytes: [UInt8]) -> Hunk? {
...
extension Hunk {
    static func from(slice: Slice<UnsafeBufferPointer<UInt8>>) -> Hunk? {
